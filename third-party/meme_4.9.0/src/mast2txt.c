...
typedef struct mast_motif MAST_MOTIF_T;
struct mast_motif {
  char *id;
  int num;
  char *name;
...
typedef struct mast_motifs MAST_MOTIFS_T;
struct mast_motifs {
  char *source;
  char *name;
  char *last_mod_date;
  int count;
  MAST_MOTIF_T **all_motifs; //in file order
  RBTREE_T *lookup; //by id
...
"\tsearches\", Bioinformatics, 14(48-54), 1998.\n"
  ...
  */
  ...
  for (i = 0; i < ps->motifs.count; i++) {
    MAST_MOTIF_T *motif = ps->motifs.all_motifs[i];
    if (motif->bad) {
      bad_motif_count++;
...
int compare_scores(const void *v1, const void *v2) {
  const SCORE_T *sc1, *sc2;
  double diff;
  int db_diff, seq_diff;
  long fp_diff;

  sc1 = (const SCORE_T*)v1;
  sc2 = (const SCORE_T*)v2;
  //order by score
...
void start_ele_motif(PARSER_STATE_T *ps, const xmlChar **attrs) {
  MAST_MOTIFS_T *motifs = &(ps->motifs);
  MAST_MOTIF_T *motif;
  int len;
  
  motif = (MAST_MOTIF_T*)mm_malloc(sizeof(MAST_MOTIF_T));
  memset(motif, 0, sizeof(MAST_MOTIF_T)); 
  motifs->count += 1;
  motifs->all_motifs = mm_realloc(motifs->all_motifs, sizeof(MAST_MOTIF_T*) * motifs->count);
...
void handle_start_doc(void *ctx) {
  ...
  //set up motifs
  ps->motifs.lookup = rbtree_create(rbtree_strcmp, NULL, NULL, NULL, NULL);
  //set up dbs
  ps->databases.lookup = rbtree_create(rbtree_strcmp, NULL, NULL, NULL, NULL);
