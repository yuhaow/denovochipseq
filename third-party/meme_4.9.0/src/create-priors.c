...
char* program_name = "createpriors";
...
static void process_command_line(
  ...
  options->usage =
    "USAGE: createpriors [options] <wiggle file>\n"
    "\n"
    "   Options:\n"
...
static void get_wiggle_info(
  ...
  *est_seq_size = 0;
  ...
  while (
    ...
    *total_score += (span * value);
    ...
    if (first_record == TRUE) {
      //  We estimate the size of the underlying sequences
      //  by summing the maximum positions for each sequence
      //  seen in the wiggle file.
      if (prev_chrom == NULL || strcmp(chrom, prev_chrom) != 0) {
        myfree(prev_chrom);
        prev_chrom = strdup(chrom);
  ...
  // Last update for estimated sequence size
  *est_seq_size += chrom_max_position;
...
int main(int argc, char *argv[]) {
  ...
  WIGGLE_READER *reader = create_wiggle_reader(options.wiggle_filename);
