...
#include "mcast_match.h" // MCAST sequence/motif-cluster matches
...
// the match doesn't contain the whole sequence?
    if (alph == DNA_ALPH) {		// save GC content
...
)    
{
  ...
  for (i = 0; i < score_set->n; ++i) {
    ...
    double pvalue = evd_set_pvalue(score, t, q, *evd_set);
    set_array_item(i, pvalue, sampled_pvalues);
  }
  sort_array(FALSE, sampled_pvalues);

  // Calculate p-values for the stored scores.
  // This array does come to us sorted.
  ARRAY_T *pvalues = allocate_array(num_matches);
  for (i = 0; i < num_matches; ++i) {
    if (matches[i]) {
      double score = get_mcast_match_score(matches[i]);
      double pvalue = evd_set_pvalue(score, t, q, *evd_set);
      set_mcast_match_pvalue(pvalue, matches[i]);
      set_mcast_match_evalue(n * pvalue, matches[i]);
  ...
  for (i = 0; i < num_matches; ++i) {
    if (matches[i]) {
      ...
      set_mcast_match_qvalue(qvalue, matches[i]);
    }
  }
...
int main (int argc, char *argv[])
{
  ...
  if (!(alph == DNA_ALPH || alph == PROTEIN_ALPH)) 
    die("%s alphabet not allowed.\n", alph_name(alph));

  // Set the PAM distance.
  if (pam_distance == -1) {
    pam_distance = (alph == DNA_ALPH ? DEFAULT_DNA_PAM : DEFAULT_PROTEIN_PAM);
  }
  if (beta < 0) beta = (alph == PROTEIN_ALPH) ? 10 : 1;
