...
static void calculate_trimmed_filtered_sequences(
  ...
  int count, i, trim_len, match, match2, similarity, max_similarity, name_len, 
      tooshort_count, nomatch_count, duplicate_count;
  ...
  *sequence_db = create_sequence_db(options->sequences_file);
  ...
  if (options->load_cismls) {
    load_spamo_primary(options->margin, options->score_threshold, background, 
        primary_motif, seqs, options->primary_cisml);
  } else {
    scan_spamo_primary(options->margin, options->score_threshold, background, 
        primary_motif, seqs);
  }
    ...
    for (index = 0; mread_has_motif(mread); ++index) {//loop over the motifs
      ...
      if (check_primary) {
        if (options->primary_name) {
          ...
          if (index == options->primary_index) goto destroy_motif;
        }
      }
      //check for inclusion
