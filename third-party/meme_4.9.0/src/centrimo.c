...
const char* CENTRIMO_USAGE = 
  ...
  "                               default: %g\n"
  "     --maxreg <width>         maximum width of any region to consider;\n"
  "                               default: use the sequence length\n"
  "     --norc                   do not scan with the reverse complement motif\n"
  "     --flip                   'flip' of sequences so that matches on the \n"
  ...
  "                               default: compute enrichment in positive sequences\n"
 ...
 *************************************************************************/
static void output_site_counts(FILE* fh, int sequence_length, MOTIF_DB_T* db, 
    MOTIF_T* motif, SITE_COUNTS_T* counts) {
  // vars
  int i, w, end;
  char *alt;
...
static void output_motif_json(JSONWR_T* json, int sequence_length,
    ...
    BOOLEAN_T discriminative, BOOLEAN_T mcc) {
  WIN_STATS_T *window;
  MOTIF_T *motif;
  MATRIX_T *freqs;
 ...
 *************************************************************************/
static void output_centrimo_text(FILE* text_file, MOTIF_STATS_T* stats,
    int motifN, int sequence_length) {
  //vars
  MOTIF_DB_T *db;
  MOTIF_T *motif;
  WIN_STATS_T *window;
  ...
  pad = 30 - strlen(get_motif_id(motif)) - 1;
  if (pad < 0) pad = 0;
  fprintf(text_file, "%s %-*s\t", get_motif_id(motif), pad, get_motif_id2(motif));
...
static MOTIF_STATS_T* calculate_best_windows(CENTRIMO_OPTIONS_T* options, 
  ...
  int n_bins, n_minus, max_win, n_windows, n_scores, n_tests, n_actual_tests;
  ...
  max_win = (options->max_win ? MIN(options->max_win, n_bins - 1) : n_bins - 1);
  n_minus = n_bins - max_win;
  n_scores = (options->optimize_score ? count_unique_scores(pve_scores, neg_scores) : 1);
  if (options->local) {
    n_windows = ((n_bins * (n_bins + 1)) / 2) - ((n_minus * (n_minus + 1)) / 2);
  } else {
  ...
  while (1) {
    ...
    if (options->local) {
      ...
      for (spread = (n_bins % 2 ? 1 : 3); spread <= max_spread; spread += 4) {
        test_window(options, log_pvalue_thresh, n_bins, log_n_tests,
            buffers->pos_c_counts, buffers->neg_c_counts, pos, spread,
            &best_ignore_count, current_windows, seqN, neg_seqN);
        n_actual_tests++;
  ...
  for (db_i = 0, i = 1; db_i < arraylst_size(options.motif_sources); db_i++) {
    ...
    for (motif_i = 0; motif_i < arraylst_size(db->motifs); motif_i++, i++) {
      ...
      DEBUG_FMT(NORMAL_VERBOSE, "Using motif %s (%d/%d) of width %d.\n", 
          get_motif_id(motif), i, motifN, get_motif_length(motif));
      // create the pssm
      ...
      if (motif_stats) {
        ...
        if (json) {
          output_motif_json(json, seqlen, motif_stats, buffers.pos_counts, 
              ...
              options.disc, options.mcc); // Write values into HTML file
        }
        output_site_counts(sites_file, seqlen, db, motif, buffers.pos_counts);
        output_centrimo_text(text_out, motif_stats, motifN, seqlen);
      }
