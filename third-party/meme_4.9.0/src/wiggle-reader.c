...
/********************************************************************
 * This function reads the next line from a WIGGLE_READER.
 ...
 * is reached.
 ********************************************************************/
static BOOLEAN_T read_wiggle_line(WIGGLE_READER *reader, char **line) {
 ...
 ********************************************************************/
static void setup_wiggle_regexp(WIGGLE_READER *reader) {
...
static void parse_wiggle_format_line(WIGGLE_READER *reader, const char *line) {
  ...
  if (reader->format == FIXED_WIGGLE_FMT) {
    ...
    *value = strtod(line, &end_str);
    ...
    if (first_record == TRUE) {
        ...
        *start = reader->start + reader->step;
        reader->start = *start;
     }
    reader->value = *value;
  }
...
WIGGLE_READER *create_wiggle_reader(const char *filename) {
    ...
    // Allocate reader
    WIGGLE_READER *reader = mm_malloc(sizeof(WIGGLE_READER));
  ...
  if (result == TRUE) {
    ...
    *first_record = FALSE;
    ...
    if (is_track_line(line) == TRUE) {
      // Skip track lines
      myfree(line);
      result = read_wiggle_line(reader, &line);
    }
    ...
    if (result == TRUE) {
      parse_wiggle_value_line(reader, line, start, value, *first_record);
      *chrom = reader->chrom;
      *step = reader->step;
      *span = reader->span;
      myfree(line);
