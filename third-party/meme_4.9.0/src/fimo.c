...
)
{
  ...
  while (get_next_block_from_data_block_reader(fasta_reader, seq_block) != FALSE) {
    ...
    if (psp_block != NULL) { 
      ...
      int psp_start = get_start_pos_for_data_block(psp_block);
      if (fwd_match.start != psp_start) {
        die(
          "Mismatch betwen sequence position and priors position at "
          "sequence coordinate %d and priors coordinate %d. \n",
          fwd_match.start,
          psp_start
        );
      }
