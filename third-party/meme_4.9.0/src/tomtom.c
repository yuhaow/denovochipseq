...
void pearson_scores(
  ...
  double x_bar = 1 / get_motif_alph_size(query_motif);
  double y_bar = 1 / get_motif_alph_size(target_motif);
...
static void print_xml_motif(FILE *file, char *prefix, MOTIF_T *motif, 
  ...
  fprintf(file, "length=\"%d\" nsites=\"%g\" evalue=\"%s\" ", len,
      get_motif_nsites(motif), log10_evalue_to_string2(log_ev, 1, buffer,
        buffer_len, 0));
  if (url && url[0] != '\0') {
    fprintf(file, "url=\"%s\"", 
...
int main(int argc, char *argv[]) {
   ...
   **********************************************/
  int option_count = 21;     // Must be updated if list below is
