...
)
{
  ...
  for (i=0; i<the_hmm->num_states; i++) { 	// state
    ...
    if (state->type == START_MOTIF_STATE && state->i_motif == i_motif) {
      pssm_matrix = allocate_matrix(state->w_motif, alph_size(alph, ALL_SIZE));
  ...
  start_state->pssm = build_matrix_pssm(alph, pssm_matrix, NULL, 0);
  ...
  if (use_both_strands) {
    ...
    start_state->npssm = build_matrix_pssm(alph, pssm_matrix, NULL, 0);
  }
  free_matrix(pssm_matrix);
...
)
{
  ...
  const int a_size = alph_size(alph, ALPH_SIZE);
  ...
  if (num_gc_bins <= 1) {			// not using GC binning
    ...
    ARRAY_T *bg = allocate_array(alph_size(alph, ALL_SIZE));
...
)
{
  ...
  int asize = get_num_cols(matrix);// could be hashed so no guarentee that it's the same size as alph_size(alph, ALPH_SIZE)

  // create the pssm
  PSSM_T* pssm = allocate_pssm(alph, w, asize, 0);
  pssm->matrix_is_log = TRUE;
...
)
{
  ...
  for (motif_offset = 0; motif_offset < motif_length; ++motif_offset) {
    best_score = get_matrix_cell(motif_offset, 0, pssm);
    for (alphabet_index = 1; alphabet_index < alphabet_size; ++alphabet_index) {
      position_score = get_matrix_cell(motif_offset, alphabet_index, pssm);
