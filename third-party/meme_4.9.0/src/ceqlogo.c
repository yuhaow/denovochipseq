...
static float CL_ghostscript_version();
/*
  Description   : Determines and then caches the version of the
                  ghostscript executable.
  Parameter     :

  Example       :
    float gs_version;
    gs_version = CL_ghostscript_version();
*/
...
void CL_add_motif(LP_PARAMS_T* params, MOTIF_T *motif, int shift, char* label) {
  ...
  if(!strcmp(format, "EPS")) {
    EPS_add(params, get_motif_freqs(motif), get_motif_nsites(motif), 
        get_motif_trim_left(motif), get_motif_trim_right(motif), 
        get_motif_alph(motif), shift, label);
  } else if(!strcmp(format, "SVG")) {
    SVG_add(params, get_motif_freqs(motif), get_motif_nsites(motif), 
        get_motif_trim_left(motif), get_motif_trim_right(motif), 
        get_motif_alph(motif), shift, label);
  } else {
    E_system("Invalid output format: %s\n", format);
...
) 
{
  ...
  CL_init(params, (get_motif_alph(motif1) == DNA_ALPH ? "NA" : "AA"), "EPS");
  if (motif1 != NULL) {
    CL_add_motif(params, motif1, shift, label1);
  }
  if (motif2 != NULL) {
    CL_add_motif(params, motif2, 0, label2);
  }
  CL_exit(params, str_internal(eps_path));
  LP_free(params);
  ...
  if (0) {
    ...
    CL_init(params, (motif1->alph == DNA_ALPH ? "NA" : "AA"), "SVG");
    if (motif1 != NULL) {
      CL_add_motif(params, motif1, shift, label1);
    }
    if (motif2 != NULL) {
...
) 
{
  double width, height;
  height = LOGOHEIGHT;
  width = get_motif_length(motif) + 2;
  if (width > MAXLOGOWIDTH) width = MAXLOGOWIDTH;
  CL_create2(motif, "", NULL, "", errbars, ssc, height, width, 0, program, path,
      make_eps, make_png);
}
...
static float CL_ghostscript_version() {
  ...
  STR_T *cmd;
  FILE *pipe;
  if (GHOSTSCRIPT_PATH == NULL) {
    fputs("No path to ghostscript given.\n", stderr); 
    version = -1;
  } else if (stat(GHOSTSCRIPT_PATH, &file_status) != 0 
      || !(S_ISREG (file_status.st_mode))
      || access(GHOSTSCRIPT_PATH, X_OK) != 0) {
...
void LP_add_line(LP_PARAMS_T* params, MATRIX_T* mat, int snum, int ltrim, 
  ...
  if((alphabet == PROTEIN_ALPH && params->is_nucleotide) ||
     (alphabet == DNA_ALPH && !params->is_nucleotide))
...
void LS_calc(int row, size_t n, MATRIX_T* mat, ALPH_T alphabet, 
  LSTACK_T* lstack, BOOLEAN_T ssc) 
{
  int    col;
  size_t asize = alph_size(alphabet, ALPH_SIZE);
  ...
  for(col=0; col<asize; col++) {
    lstack[col].letter = alph_char(alphabet, col);
...
void LS_test() {
  T_start();
  ALPH_T alphabet = DNA_ALPH;
  {
    int asize = alph_size(alphabet, ALPH_SIZE);
    MATRIX_T* mat = allocate_matrix(2, asize);
    set_matrix_cell(0,0, 0.1, mat);
    set_matrix_cell(0,1, 0.7, mat);
    set_matrix_cell(0,2, 0.1, mat);
    set_matrix_cell(0,3, 0.1, mat);
    {
      T_double(2.00, LS_calc_Hmax(asize), 0.01);
    }
    {
      T_double(1.35, LS_calc_H(0,mat,asize), 0.01);
    }
    {
      T_double(0.00, LS_calc_e(0,asize), 0.01);
      T_double(0.21, LS_calc_e(10,asize), 0.01);
    }
    {
      T_double(0.64, LS_calc_R(0,0,mat,asize, TRUE), 0.01);
    }
    {
      T_double(0.064, LS_calc_height(0,0,0,mat,asize, TRUE), 0.001);
    }
    free_matrix(mat);
  }
  T_end();
...
void SVG_add(LP_PARAMS_T* params, MATRIX_T* mat, int snum, 
    int ltrim, int rtrim, ALPH_T alphabet, int shift, char* label) {
...
void EPS_add_motifs(LP_PARAMS_T* params, MOTIF_T* motifs, int num_motifs, ALPH_T alphabet) {
  int i;
  MOTIF_T *motif;
...
void EPS_exit(LP_PARAMS_T* params, FILE *fout) {
  ...
  for(lnr=0; lnr<params->l_num; lnr++) {         /* Over all logo lines */
    ...
    LP_LINE_T* line  = LP_get_line(params, lnr);
    MATRIX_T*  mat   = line->mat;
    int        asize = alph_size(line->alphabet, ALPH_SIZE);
...
void CL_load_motifs(LP_PARAMS_T* params, char* mode, char *filename, float pseudocounts) {
  ...
  mread_set_pseudocount(mread, pseudocounts);
  ...
  while (mread_has_motif(mread)) {
    ...
    if (index < 0 || index == i) CL_add_motif(params, motif, 0, NULL);
    destroy_motif(motif);
    ++i;
  }
  if (i <= index) CL_error("Invalid motif index: %d\n", index+1);
  mread_destroy(mread);
}
