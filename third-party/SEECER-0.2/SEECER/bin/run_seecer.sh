...
#        ${BINDIR}/order_reads $Read1_N $Read2_N  --kmer $K -k $TMPDIR/counts_${K}_${LCOUNT} -o $TMPDIR/order.fasta -i $TMPDIR/order_index --kmerOut $TMPDIR/debug/kmerGraph
	echo "finished order reads: " `date`;
	python ${BINDIR}/reorder.py $TMPDIR/reorder.fasta $TMPDIR/reorder_index $TMPDIR/reorder_new.fasta $TMPDIR/reorder_new_index > $TMPDIR/reorder_log
	echo "finished reorder reads: " `date`;
        ${BINDIR}/seecer $TMPDIR/reorder_new.fasta $SEECER_PARAMS -i $TMPDIR/reorder_new_index --kmer $K -k $TMPDIR/counts_${K}_${LCOUNT} -o $TMPDIR/corrected.fasta
#        ${BINDIR}/seecer $TMPDIR/order.fasta $SEECER_PARAMS -i $TMPDIR/order_index --kmer $K -k $TMPDIR/counts_${K}_${LCOUNT} -o $TMPDIR/corrected.fasta
    else
