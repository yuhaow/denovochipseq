...
then
    Read2_N="${Read2}"
    RS_ARGS="$RS_ARGS $Read2,${Read2}"
    Reads="$Reads,$Read2"
    Reads_N="${Reads_N},${Read2}"
...
then
    ...
    echo
    bash ${BINDIR}/run_jellyfish.sh $JF $TMPDIR/counts_${K}_${LCOUNT} $K $LCOUNT $TMPDIR $Read1 $Read2
fi;
...
#        ${BINDIR}/order_reads $Read1_N $Read2_N  --kmer $K -k $TMPDIR/counts_${K}_${LCOUNT} -o $TMPDIR/reorder.fasta -i $TMPDIR/reorder_index
        ${BINDIR}/order_reads $Read1 $Read2  --kmer $K -k $TMPDIR/counts_${K}_${LCOUNT} -o $TMPDIR/order.fasta -i $TMPDIR/order_index --kmerOut $TMPDIR/debug/kmerGraph
    else
