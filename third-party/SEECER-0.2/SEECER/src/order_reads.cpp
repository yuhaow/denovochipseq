...
uint64_t mergeCC(PARENT_MAP& cc_links, uint64_t cc1, uint64_t cc2) {
  ...
  if (cc1 != cc2) {
    ...
    /*
    if (cc1 != cc1p.first) {
      std::cerr << "BAD: " << cc1 << " " << cc1p.first << std::endl;
    }

    if (cc2 != cc2p.first) {
      std::cerr << "BAD: " << cc2 << " " << cc2p.first << std::endl;
    }
    */
...
int order_reads(int argc, char * argv[])
{
	...
	static struct option long_options[] =
		...
		{"kmerOutput", required_argument, 0, '6'},
		{0, 0, 0, 0}
	    };
	int option_index = 0;
    ...
    // making sure the kmer is valid
    std::ofstream kmerOutput(kmerOutputFn);
    ...
    if (!kmerOutput.is_open()) {
        std::cerr << "Kmer output file is invalid!!!" << std::endl;
        return -1;
    }
...
#pragma omp parallel for
	  ...
	  if (CCs.size() > 1) {
	    ...
	    std::set<uint64_t>::const_iterator it = CCs.begin();
	    while (it != CCs.end()) {
	      std::set<uint64_t>::const_iterator elm = it++;
	      if (kmer_assigns[*elm] != readidx) {
		CCs.erase(elm);
	      }
	    }
...
#pragma omp atomic
    ...
    uint64_t *fReads = static_cast<uint64_t*>(calloc(total_reads, sizeof(uint64_t)));
	  ...
	  if (!fProcessedReads[readidx]) {
	    if (pairedEnd) {
	      ...
	      output << ">" << fragStore.readNameStore[readidx] << std::endl
		     << fragStore.readSeqStore[readidx] << std::endl;
#if DEBUG
	      kmerOutput << ">" << fragStore.readNameStore[readidx] << std::endl;
	      std::set<uint64_t> lkmers;
	      finder->ConstructGramSet(fragStore.readSeqStore[readidx], lkmers);
	      for (std::set<uint64_t>::const_iterator it = lkmers.begin(); it != lkmers.end(); ++it) {
	          kmerOutput << *it << "\t";
	      }
	      kmerOutput << std::endl;
#endif
	    ...
	    fReads[readidx] = it->first;
	  }

	  if (fReads[readidx] != it->first) {
	    std::cerr << readidx << " assigned to different CC !!!!" << finder->GramToString(it->first) << " != "
		      << finder->GramToString(fReads[readidx]) << std::endl;
	  }

	  assert(fReads[readidx] == it->first);
        }
