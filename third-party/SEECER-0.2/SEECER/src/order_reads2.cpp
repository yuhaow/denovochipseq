...
#pragma omp parallel for
	...
	if (kmer_assigns.find(*it) != kmer_assigns.end() ) {
	  ...
	  if (coin) {
	    ...
	    if (mainCC == 1477788167213171208L) {
	      std::cerr << mainCC << " " << getCC(cc_links, mainCC) << std::endl;
	    }
	    
	  for (;it != CCs.end(); ++it) {

	  if (*it == 1477788167213171208L) {
	    std::cerr << *it << " " << getCC(cc_links, *it)
		    << " " << kmer_kmer_assigns[*it] << std::endl;
	  }
