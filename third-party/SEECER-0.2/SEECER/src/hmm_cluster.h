...
#define _MARGIN_OK_(f, x) ( (x <= 3 && f>=0.3) || ( x> 3 && f >= 0.01) || x >= 10 )
//( (x <= 3 && f>=0.5) || ( x> 3 && f >= 0.01) || x >= 10 )
typedef struct Emission {
  ...
  bool isDisambiguous() const {
    ...
    return _MARGIN_OK_(exp(s[letter]) - exp(m), c[NDNA]);
  }
...
private:
    ...
    int ExtendLeft(DnaString& core,
		   ...
		   int buffer_length,
		   std::ostream* os);
    int ExtendLeftStart(DnaString& core,
		   PositionInfo& pos,
		   Emission* buffer,
		   int buffer_iter,
		   int buffer_length,
    ...
    int ExtendRight(DnaString& core,
		    ...
		    int buffer_length,
		    std::ostream* os);
    int ExtendRightStart(DnaString& core,
		    PositionInfo& pos,
		    Emission* buffer,
		    int buffer_iter,
		    int buffer_length,
