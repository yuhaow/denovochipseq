...
//	 extended_left_length = MIN(extended_left_length, 15);
//	 extended_right_length = MIN(extended_right_length, 15);
	 ...
	 if (idx[2] <= idx[1]) {
	   ...
	   if (idx[2] < extended_left_length)
		break;
	   idx[0] = extended_left_length;
	   if (idx[2] == extended_left_length){
		GatherReads(readid, main_core, main_pos, idx, 2, rthread,
		       emission_buffer, it_emission_buffer,
		       length_emission_buffer);
	   } else {
		idx[3] = static_cast<int>(length(main_core)) - extended_right_length;
